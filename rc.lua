...
tags_name   = { "im", "www", "term", "00", "01", "10", "11" }
-- }}}
...
function calendar:month(month_offset)
    ...
    local cal = awful.util.pread("cal -h " .. datespec)
    if self.offset == 0 then -- this month, hilight day and month
        cal = string.gsub(cal, "%s" .. tostring(os.date("%d")) .. "%s", italic(bold(fg(beautiful.hilight, "%1"))))
    ...
    self.display = naughty.notify {
        ...
        text = string.format('<span font_desc="%s">%s</span>', theme.font_mono, cal),
        timeout = 0,
        hover_timeout = 0.5,
        margin = 10,
    }
end
-- removing the pop-up calendar
...
for s = 1, screen.count() do
  ...
  for id, name in ipairs(tags_name) do
    ...
    tags[s][id].screen = s
    if id == 1 then
      awful.layout.set(layouts[9], tags[s][id])
    else
...
clockbox:add_signal("mouse::enter", function() calendar:month(0) end)
clockbox:add_signal("mouse::leave", function() calendar:remove() end)
clockbox:add_signal("update", function() clockbox.text = widget_clock() end)
...
-- {{{ Client manipulation, global part
  ...
  awful.key(k_ms, "j",   function () awful.screen.focus_relative( 1) end),
  awful.key(k_ms, "k",   function () awful.screen.focus_relative(-1) end),
...
--  awful.key(k_ma, "Left", awful.client.movetotag(tags[client.focus.screen][i]),
--  awful.key(k_ma, "Prior", awful.client.movetotag(tags[client.focus.screen]tags[screen-1]),
-- }}}
...
-- {{{ Prompts
  ...
  awful.key(k_m, "x", function () mypromptbox[mouse.screen]:run() end),
...
clientkeys = awful.util.table.join(
  ...
  awful.key(k_mc, "k",      function (c) c:kill() end),
